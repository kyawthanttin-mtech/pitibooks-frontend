...
const BillsEdit = () => {
  ...
  const [addProductsModalOpen, setAddPurchaseProductsModalOpen] =
    useState(false);
  const [subTotal, setSubTotal] = useState(record?.billSubtotal);
  ...
  const [updateBill, { loading: updateLoading }] = useMutation(UPDATE_BILL, {
    onCompleted() {
      openSuccessMessage(
        msgApi,
        <FormattedMessage id="bill.updated" defaultMessage="Bill Updated" />
      );
  ...
  const onFinish = async (values) => {
    ...
    await updateBill({
      ...
      update(cache, { data: { updateBill } }) {
        cache.modify({
          fields: {
            paginateBill(pagination = []) {
              const index = pagination.edges.findIndex(
                (x) => x.node.__ref === "Bill:" + updateBill.id
              );
              if (index >= 0) {
                const newBill = cache.writeFragment({
                  data: updateBill,
                  fragment: gql`
                    fragment NewBill on Bill {
                      id
                      supplier
                      branch
                      purchaseOrderNumber
                ...
                paginationCopy.edges[index].node = newBill;
    ...
    <>
      ...
      <div className="page-header">
        <p className="page-header-text">
          <FormattedMessage id="bill.edit" defaultMessage="Edit Bill" />
        </p>
        <Button
          icon={<CloseOutlined />}
          type="text"
