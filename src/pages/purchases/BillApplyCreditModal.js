...
import { SupplierQueries, SupplierMutations, BillQueries } from "../../graphql";
...
const BillApplyCreditModal = ({
  ...
  const [applyCredit, { loading: applyLoading }] = useMutation(
    CREATE_SUPPLIER_APPLY_CREDIT,
    {
      ...
      update: (cache, { data: { createSupplierApplyCredit } }) => {
        ...
        if (existingData) {
          ...
          if (
            ...
            const updatedSelectedRecord = {
              ...selectedRecord,
              appliedSupplierCredits: [
                ...selectedRecord.appliedSupplierCredits,
                ...createSupplierApplyCredit,
              ],
            };
  ...
  const columns = [
    {
      ...
      render: (_, record) => (
        <>
          <div>
            {record.type === "Credit" ? record.creditDetails : "Advance"}
          </div>
...
export default BillApplyCreditModal;
