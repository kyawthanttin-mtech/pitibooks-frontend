...
import { SupplierMutations } from "../../graphql";
const { UPDATE_SUPPLIER } = SupplierMutations;
  ...
  const onFinish = async (values) => {
    ...
    await updateSupplier({
      ...
      update(cache, { data: { updateSupplier } }) {
        cache.modify({
          fields: {
            paginateSupplier(pagination = []) {
              const index = pagination.edges.findIndex(
                (x) => x.node.__ref === "Supplier:" + updateSupplier.id
              );
              if (index >= 0) {
                const newSupplier = cache.writeFragment({
                  data: updateSupplier,
                  fragment: gql`
                ...
                paginationCopy.edges[index].node = newSupplier;
