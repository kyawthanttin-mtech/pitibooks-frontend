...
const { UPDATE_CUSTOMER } = CustomerMutations;
...
const CustomersEdit = () => {
  ...
  const [selectedSAState, setSelectedSAState] = useState(
    record?.shippingAddress.stateId
  );

  const [data, setData] = useState(
    record?.contactPersons?.map((_, index) => ({ key: index })) || [{ key: 0 }]
  );
    ...
    const parsedRecord =
      record && record.contactPersons
        ? {
            ...
            notes: record?.notes,
            // Billing Address
            b_attention: billingAddress?.attention,
            b_country: billingAddress?.country,
            b_stateId: billingAddress?.stateId,
            b_city: billingAddress?.city,
            b_townshipId: billingAddress?.townshipId,
            b_address: billingAddress?.address,
            b_email: billingAddress?.email,
            b_phone: billingAddress?.phone,
            b_mobile: billingAddress?.mobile,
            // Shipping Address
            s_attention: shippingAddress?.attention,
            s_country: shippingAddress?.country,
            s_stateId: shippingAddress?.stateId,
            s_city: shippingAddress?.city,
            s_townshipId: shippingAddress?.townshipId,
            s_address: shippingAddress?.address,
            s_email: shippingAddress?.email,
            s_phone: shippingAddress?.phone,
            s_mobile: shippingAddress?.mobile,
            // Map transactions to form fields
            ...record.contactPersons.reduce((acc, cp, index) => {
              acc[`name${index}`] = cp.name;
              acc[`email${index}`] = cp.email;
              acc[`phone${index}`] = cp.phone;
              acc[`mobile${index}`] = cp.mobile;
              acc[`designation${index}`] = cp.designation;
              acc[`department${index}`] = cp.department;
              return acc;
            }, {}),
          }
        : {};
  ...
  const states = useMemo(() => {
    return stateData?.listAllState;
  }, [stateData]);

  const townships = useMemo(() => {
    return townshipData?.listAllTownship;
  }, [townshipData]);
  ...
  const onFinish = async (values) => {
    ...
    await updateCustomer({
      ...
      update(cache, { data: { updateCustomer } }) {
        cache.modify({
          fields: {
            paginateCustomer(pagination = []) {
              const index = pagination.edges.findIndex(
                (x) => x.node.__ref === "Customer:" + updateCustomer.id
              );
              if (index >= 0) {
                const newCustomer = cache.writeFragment({
                  data: updateCustomer,
                  fragment: gql`
                ...
                paginationCopy.edges[index].node = newCustomer;
  ...
  return (
    <>
      <div className="page-header">
        <p className="page-header-text">Edit Customer</p>
        <Button
          icon={<CloseOutlined />}
          type="text"
          onClick={() =>
