...
const InvoicesEdit = () => {
  ...
  const [addProductsModalOpen, setAddPurchaseProductsModalOpen] =
    useState(false);
  const [subTotal, setSubTotal] = useState(record?.invoiceSubtotal);
  ...
  const onFinish = async (values) => {
    ...
    await updateInvoice({
      ...
      update(cache, { data: { updateBill } }) {
        cache.modify({
          fields: {
            paginateSalesInvoice(pagination = []) {
              const index = pagination.edges.findIndex(
                (x) => x.node.__ref === "Invoice:" + updateInvoice.id
              );
              if (index >= 0) {
                ...
                paginationCopy.edges[index].node = newInvoice;
    ...
    <>
      ...
      <div className="page-header">
        <p className="page-header-text">
          <FormattedMessage id="invoice.edit" defaultMessage="Edit Invoice" />
        </p>
        <Button
          icon={<CloseOutlined />}
          type="text"
