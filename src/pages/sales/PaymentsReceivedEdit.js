...
const { UPDATE_CUSTOMER_PAYMENT } = CustomerPaymentMutations;

const initialValues = {
  paymentDate: dayjs(),
};

const PaymentsReceivedEdit = () => {
  const intl = useIntl();
  ...
  const [selectedCustomer, setSelectedCustomer] = useState(record?.customer);
  ...
  useMemo(() => {
    const parsedRecord = record
      ? {
          ...
          paidThrough: record.depositAccount?.id || null,
  ...
  // Mutations
  const [updateCustomerPayment, { loading: updateLoading }] = useMutation(
    UPDATE_CUSTOMER_PAYMENT,
    {
      onCompleted() {
        openSuccessMessage(
          msgApi,
          <FormattedMessage
            id="customerPayment.updated"
            defaultMessage="Customer Payment Updated"
          />
        );
  ...
  const onFinish = async (values) => {
    try {
      ...
      await updateCustomerPayment({
        ...
        update(cache, { data: { updateCustomerPayment } }) {
          cache.modify({
            fields: {
              paginateCustomerPayment(pagination = []) {
                const index = pagination.edges.findIndex(
                  (x) =>
                    x.node.__ref ===
                    "CustomerPayment:" + updateCustomerPayment.id
                );
                if (index >= 0) {
                  const newCustomerPayment = cache.writeFragment({
                    data: updateCustomerPayment,
                    fragment: gql`
                      fragment NewCustomerPayment on CustomerPayment {
                        id
                        branch
                        customer
                        currency
                  ...
                  paginationCopy.edges[index].node = newCustomerPayment;
...
export default PaymentsReceivedEdit;
