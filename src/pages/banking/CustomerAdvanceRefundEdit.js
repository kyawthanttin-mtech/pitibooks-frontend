...
const CustomerAdvanceRefundEdit = ({
  refetch,
  modalOpen,
  setModalOpen,
  branches,
  ...
  const [selectedAdvance, setSelectedAdvance] = useState(
    selectedRecord?.customer?.availableAdvances
  );
    ...
    const parsedRecord =
      form && modalOpen && selectedRecord
        ? {
            ...
            refundDate: dayjs(selectedRecord.refundDate),
        ...
        : {};
    setSelectedCustomer(selectedRecord?.customer);
    setSelectedBranch(selectedRecord?.branch?.id);
    setFromAccountCurrencyId(selectedRecord?.fromAccount?.currency?.id || null);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!values[`refundAmount`] || values[`refundAmount`] < 0) {
        openErrorNotification(
          notiApi,
          intl.formatMessage({
            id: "validation.pleaseSelectAnAdvance",
      ...
      await updateRefund({ variables: { id: selectedRecord?.id, input } });
      setModalOpen(false);
      form.resetFields();
    } catch (err) {
...
export default CustomerAdvanceRefundEdit;
