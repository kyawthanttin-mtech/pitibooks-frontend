...
const DepositFromAnotherAccEdit = ({
  refetch,
  modalOpen,
  setModalOpen,
  branches,
  ...
  const handleFromAccountChange = useCallback(
    (id) => {
      const toAccountCurrency =
        selectedRecord?.toAccount?.currency?.id > 0
          ? selectedRecord?.toAccount?.currency
  ...
  const handleSubmit = async () => {
    try {
      ...
      const input = {
        ...values,
        transactionType: "DepositFromOtherAccounts",
        // isMoneyIn: true,
        documents: fileUrls,
      };
...
export default DepositFromAnotherAccEdit;
