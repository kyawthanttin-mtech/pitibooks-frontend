...
const PaymentReceivedPDF = ({ selectedRecord, business }) => {
  const data = selectedRecord?.paidInvoices ? selectedRecord?.paidInvoices : [];
  ...
  const Title = () => (
    <View style={styles.titleSection}>
      ...
      <CustomText style={styles.alignRight}>
        <CustomText style={styles.header}>PAYMENTS RECEIVED{"\n"}</CustomText>
        <CustomText>
      ...
      <View style={[styles.contentDetails, styles.alignRight]}>
        ...
        >
          <CustomText style={styles.alignRight}>
            ...
            {selectedRecord?.paymentMode?.id > 0 && (
              <CustomText>Payment Mode :{"\n"}</CustomText>
            )}
            <CustomText>Bank Charges :{"\n"}</CustomText>
            {selectedRecord?.depositAccount?.id > 0 && (
        ...
        <View style={styles.lineHeight2}>
          <CustomText style={styles.alignRight}>
            <CustomText>
              {dayjs(selectedRecord?.customerPaymentDate).format(
                REPORT_DATE_FORMAT
              )}
              {"\n"}
            </CustomText>
            ...
            {selectedRecord?.depositAccount?.id > 0 && (
              <CustomText>
                {selectedRecord?.depositAccount?.name}
                {"\n"}
              </CustomText>
            )}
          </CustomText>
  ...
  const TableHead = () => (
    <View style={styles.thead} fixed>
      <View style={[styles.headerRow, { flex: 1, paddingLeft: 10 }]}>
        <CustomText style={styles.alignLeft}>Invoice Number</CustomText>
      </View>
      <View style={[styles.headerRow, { flex: 1, paddingLeft: 10 }]}>
        <CustomText>Invoice Date</CustomText>
      </View>
      <View style={[styles.headerRow, { flex: 1, paddingRight: 10 }]}>
        <CustomText style={styles.alignRight}>Invoice Amount</CustomText>
      </View>
      <View
        style={[
          styles.headerRow,
      ...
      <View wrap>
        ...
        {data.map((item, index) => (
          <View wrap={false} key={index} style={styles.bodyRow}>
            ...
            <View style={[{ flex: 1, padding: 10 }]}>
              <CustomText>
                {dayjs(item.invoice?.invoiceDate).format(REPORT_DATE_FORMAT)}
              </CustomText>
            </View>
            <View style={[{ flex: 1, padding: 10 }]}>
              <CustomText style={styles.alignRight}>
                <FormattedNumber
                  value={item.invoice?.invoiceTotalAmount}
                  style="decimal"
                  minimumFractionDigits={selectedRecord.currency.decimalPlaces}
                />
              </CustomText>
  ...
  const BalanceDetail = () => (
    <View style={[styles.balanceDetails]}>
      <CustomText style={[styles.alignRight, { paddingRight: 11 }]}>
        ...
        {selectedRecord.customerPaymentDiscountAmount > 0 && (
          <CustomText>
            Discount{" "}
            {selectedRecord.customerPaymentDiscountType === "P" &&
              "(" + selectedRecord.customerPaymentDiscount + "%)"}
            {"\n"}
          </CustomText>
        )}
        {selectedRecord.customerPaymentTotalTaxAmount > 0 && (
          <CustomText>
      ...
      <CustomText style={styles.alignRight}>
        <CustomText>
          <FormattedNumber
            value={selectedRecord?.customerPaymentSubtotal}
            style="decimal"
            minimumFractionDigits={selectedRecord?.currency.decimalPlaces}
          />
          {"\n"}
        ...
        {selectedRecord.customerPaymentDiscountAmount > 0 && (
          <CustomText>
            -
            <FormattedNumber
              value={selectedRecord?.customerPaymentDiscountAmount}
              style="decimal"
              minimumFractionDigits={selectedRecord?.currency.decimalPlaces}
            />
            {"\n"}
        ...
        {selectedRecord.customerPaymentTotalTaxAmount > 0 && (
          <CustomText>
            <FormattedNumber
              value={selectedRecord?.customerPaymentTotalTaxAmount}
              style="decimal"
              minimumFractionDigits={selectedRecord?.currency?.decimalPlaces}
            />
            {"\n"}
        ...
        <CustomText style={[styles.boldText]}>
          {selectedRecord?.currency?.symbol}{" "}
          <FormattedNumber
            value={selectedRecord?.customerPaymentTotalAmount}
            style="decimal"
            minimumFractionDigits={selectedRecord?.currency?.decimalPlaces}
          />
          {"\n"}
